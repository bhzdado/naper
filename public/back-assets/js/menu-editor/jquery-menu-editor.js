...
function MenuEditor(idSelector, options) {
    var $main = $("#" + idSelector);
    var settings = {
        labelEdit: '<i class="fas fa-edit clickable btnEditMenu"></i>',
        labelRemove: '<i class="fas fa-trash-alt clickable btnEditRemove"></i>',
        textConfirmDelete: 'Você tem certeza que deseja remover este item?',
    ...
    function createMenu(arrayItem, depth) {
        ...
        $.each(arrayItem, function (k, v) {
            ...
            var itemObject = {text: "", href: "", icon: "empty", target: "_self", title: "", "published":"0", "order": ""};
            var temp = $.extend({}, v);
            if (isParent){ 
