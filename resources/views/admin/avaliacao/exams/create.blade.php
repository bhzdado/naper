...
<div class="row" style="margin:10px;">
    <div class="col-lg-12">
        <form class="form-horizontal" name="form_exam_create" id="form_exam_create">
            {{ csrf_field() }}
            @if (isset($exam))
            <input type="hidden" id='route' value='exam/update/{{ $exam->id }}'>
            <input type="hidden" id='method' value='patch'>
            @else
            <input type="hidden" id='route' value='exam/store/'>
            ...
            <div class="card-body">
                <div class="form-row">
                    <div class="col-md-3 mb-3">
                        <label>ID: </label>
                        @if (isset($exam))
                        <input type="text" class="form-control" id="validationServer01" value="{{ str_pad( $exam->id , 8, "0", STR_PAD_LEFT) }}"  disabled="disabled">
                        @else
                ...
                <div class="form-row">
                    <div class="col-md-9 mb-3">
                        <label>Título:</label>
                        <input type="text" id="name" name="name" class="form-control" style="" value="{{ (isset($exam)) ? $exam->name : '' }}">
                    </div>
                    <div class="col-md-3 mb-3">
                        <label>Valor:</label>
                        <input type="text" id="value" name="value" class="form-control" style="" value="{{ (isset($exam)) ? $exam->value : '0' }}">
                    </div>
                </div>
                <div class="form-row">
                    <div class="col-md-4 mb-3">
                        <label>Tempo Máximo (minutos):</label>
                        <input type="text" id="maximum_time" name="maximum_time" class="form-control" style="" value="{{ (isset($exam)) ? $exam->maximum_time : '' }}">
                    </div>
                    <div class="col-md-4 mb-3">
                        <label>Data Início Avaliação:</label>
                        <input type="text" id="start_time" name="start_time" class="form-control mask-date datepicker" style="" value="{{ (isset($exam)) ? $exam->start_time : '' }}">
                    </div>
                    <div class="col-md-4 mb-3">
                        <label>Data Limite Avaliação:</label>
                        <input type="text" id="finish_time" name="finish_time" class="form-control mask-date datepicker" style="" value="{{ (isset($exam)) ? $exam->finish_time : '' }}">
                    </div>
                </div>
                <div class="form-row">
                    <div class="col-md-3 mb-3">
                        <label>Correção em Tempo Real?</label>
                        <select class="form-control" name="real_time_correction" id="real_time_correction"  style="">
                            <option value="0" {{ (isset($exam->real_time_correction) && $exam->real_time_correction == 0) ? "selected:selected" : "" }} > Não </option>
                            <option value='1' {{ (isset($exam->real_time_correction) && $exam->real_time_correction == 1) ? "selected:selected" : "" }} > Sim </option>
                        </select>
                    </div>
                    <div class="col-md-3 mb-3">
                        <label>Publicardo?</label>
                        <select class="form-control" name="published" id="published"  style="">
                            <option value="0" {{ (isset($exam->published) && $exam->published == 0) ? "selected:selected" : "" }} > Não </option>
                            <option value='1' {{ (isset($exam->published) && $exam->published == 1) ? "selected:selected" : "" }} > Sim </option>
                        </select>
                    </div>
                </div>
            ...
            <div class="form-actions" style='margin-top:15px;'>
                ...
                @ENDIF
                <input type="button" value="Voltar" class="btn btn-success"  onclick="openRoute('avaliacao/exam');">
            </div>
        </form>
    </div>
</div>
...
<script src="{{asset('js/avaliacao/exams.js')}}"></script> 
@IF (isset($action) && $action == 'show')
<script>
    $(document).ready(function () {
        $('input:not([type=button])').attr('disabled', 'disabled');
        $('select').attr('disabled', 'disabled');
        CKEDITOR.instances['exam'].config.readOnly = true;
    });
</script>
@ENDIF